...
BG_COLOR   = (153, 204, 255)   # hele sinine nagu pildil
TEXT_COLOR = (60, 60, 60)
...
def draw_ball():
    ...
    if use_ball_img:
        ...
        pygame.draw.circle(screen, (255, 165, 0), (int(ball_x), int(ball_y)), BALL_SIZE // 2)
...
def draw_pad():
    ...
    if use_pad_img:
        ...
        pygame.draw.rect(screen, (139, 90, 43), (px, PAD_Y, PAD_W, PAD_H), border_radius=6)
...
def reset():
    global ball_x, ball_y, ball_vx, ball_vy, pad_x, pad_vx, score, game_over
    ball_x = float(SCREEN_W // 2)
    ball_y = float(SCREEN_H // 3)
    ball_vx = float(BALL_SPEED_X)
    ball_vy = float(BALL_SPEED_Y)
